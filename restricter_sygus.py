...
def solve(entity_schema, actions, action_scope, hierarchy, entity_store_json, log, orig_policy, args, verbose=False, vv=False, synth_rule = None, compl=None, eq=False, synth_conj_only=True):
    mode = None
    if  "num_allows" in args and args["num_allows"] is not None and args["num_denies"] is not None:
        mode = SynthMode.MULTIPLE_RULES
    elif "num_conj" not in args or (args["num_conj"] is not None and args["num_conj"] > 0):
    ...
    # FOL translation of permit rule
    ...
    if mode == SynthMode.MULTIPLE_RULES:
        ...
        # Synthesize one rule with conjunctions
    ...
    for attr_name, _ in get_attrs:
        # resource or principal has attr_name
        for e in [principal, resource]:
            has_attr_term = slv.mkTerm(Kind.APPLY_UF, has_attr, e, slv.mkString(attr_name))
            entity_ins_grammar.append(has_attr_term)
    ...
    if compl is not None:
        ...
        if len(negation_predicate_lst) == 1:
            ...
            if verbose: print(f"Negating point in POP: {negation_predicate_lst[0]}")
        else:
            negation_predicate = slv.mkTerm(Kind.OR, *negation_predicate_lst)
            slv.addSygusConstraint(negation_predicate)
            if verbose: print(f"Negating point in POP: {negation_predicate}")
    else:
        # No complement points provided: state the restrictiveness constraint
        # For all p, a, r, if original_policy(p, a, r) = False => policies(p, a, r) = False
        restrictiveness_constraint = slv.mkTerm(
            Kind.IMPLIES, slv.mkTerm(Kind.NOT, encoded_rules[synth_rule].substitute([negative_example_p, negative_example_a, negative_example_r], [principal, action, resource])),
            slv.mkTerm(Kind.NOT, slv.mkTerm(Kind.APPLY_UF, policies, principal, action, resource, ctx))
            )
        slv.addSygusConstraint(restrictiveness_constraint)
        if verbose:
            print("Added restrictiveness constraint: ", restrictiveness_constraint)
...
# Converts synthesized rules as SMT terms back to cedar policy
class Unparser():
    ...
    def rule_to_str(self, rule):
        match rule.getKind():
            ...
            case Kind.CONST_BOOLEAN:
                return "true" if rule.toPythonObj() else "false"
            case Kind.ITE:
                return f"if ({self.rule_to_str(rule[0])}) then ({self.rule_to_str(rule[1])}) else ({self.rule_to_str(rule[2])})"
            case _:
                print(rule.getKind())
                print(rule)
