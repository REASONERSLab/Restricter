...
from util import transitive_closure
from parse_schema import ground_depth
from cedar2json import to_json, rule_to_json
...
class Solver():
    """SMT Solver for generating requests that satisfy a cedar rule."""

    def __init__(self, entity_schema, actions, action_scope, hierarchy, entity_store_json, orig_policy, verbose=False, vv=False, seed=None, entity_random_seed=None):
        ...
        seed = seed if seed is not None else int(datetime.now().timestamp())
        slv.setOption("sat-random-seed", str(seed))
        ...
        if entity_random_seed is not None:
            random.seed(entity_random_seed)
            random.shuffle(entity_store_json)
        ...
        ## rule constants
        self.principal = self.slv.mkConst(self.entity_sort, "principal")
        self.action = self.slv.mkConst( self.string_sort, "action")
        self.resource = self.slv.mkConst(self.entity_sort, "resource")
        ...
        for policy in policy_json:
            ...
            p_restriction, a_restriction, r_restriction, metadata = self.parse_header(policy)
    ...
    def parse_primary(self, primary_json):
        ...
        match primary_json['stype']:
            case 'literal':
                ...
                match val['type']:
                    case 'bool':
                        return self.slv.mkBoolean(val['val'] == 'true')
                    case _:
                        raise NotImplementedError(f"Literal of type {val['type']} not implemented")
            case 'ref':
                val = primary_json['val']
                path = val["path"]
                eid = val["eid"]
            ...
            case 'eList':
                pass
            case 'list':
                items = [self.parse_expr(entry) for entry in primary_json['val']]
                item_terms = [self.slv.mkTerm(Kind.SET_SINGLETON, item) for item in items]
                return reduce(lambda a, b: self.slv.mkTerm(Kind.SET_UNION, a, b), item_terms)
    ...
    def parse_rule(self, rule_text):
        rule_json = json.loads(rule_to_json(rule_text))
        return self.parse_rule_from_json(rule_json)
    
    def parse_rule_from_json(self, policy):
        effect = policy["effect"]
        p_restriction, a_restriction, r_restriction, metadata = self.parse_header(policy)
        assert(len(policy["conditions"]) <= 1)
        cond = self.parse_expr(policy["conditions"][0]) if len(policy["conditions"]) == 1 else self.slv.mkTrue()
        if effect == "Permit":
            return self.slv.mkTerm(Kind.AND, p_restriction, a_restriction, r_restriction, cond)
        else:
            return self.slv.mkTerm(Kind.IMPLIES, self.slv.mkTerm(Kind.AND, p_restriction, a_restriction, r_restriction), self.slv.mkTerm(Kind.NOT, cond))


    def parse_header(self, policy):
        slv = self.slv
                    ...
                    case "In":
                        ...
                        match expr["stype"]:
                            ...
                            case 'list':
                                ...
                                action_restriction = slv.mkTerm(Kind.EQUAL, target, slv.mkString(self.actions[0]))
                                for a in _actions[1:]:
        ...
        metadata["p_restrict"] = p_restriction
        metadata["a_restrict"] = a_restriction
        metadata["r_restrict"] = r_restriction
        return p_restriction, a_restriction, r_restriction, metadata
                ...
                match ext['rel']:
                    ...
                    case "NotEq":
                        if rval.getSort() == self.entity_sort:
                            return self.slv.mkTerm(Kind.NOT, self.slv.mkTerm(Kind.APPLY_UF, self.entity_eq, initial, rval))
                        else:
                            return self.slv.mkTerm(Kind.NOT, self.slv.mkTerm(Kind.EQUAL, initial, rval))
                    case "Eq":
                        if rval.getSort() == self.entity_sort:
    ...
    def parse_expr(self, expr_json):
        ...
        match expr_json['stype']:
            ...
            case 'or': # A disjunction of expressions
                ...
                return rv
            case 'ite': # if-then-else expression
                if_expr = self.parse_expr(expr_json['if'])
                then_expr = self.parse_expr(expr_json['then'])
                else_expr = self.parse_expr(expr_json['else'])
                return self.slv.mkTerm(Kind.ITE, if_expr, then_expr, else_expr)
        raise ValueError()

    def get_pt(self, log, synth_rule, check_log=False):
        """ Given a log of access requests, and a rule to synthesize, return a PT for that rule not in the log."""
        self.slv.push()
        ...
        if r2.isSat():
            ...
            unsat_core = self.slv.getUnsatCore()
            self.slv.pop()
            return False, unsat_core

    def get_pt_generator_term(self, log, term, check_log=False, mode=cvc5.BlockModelsMode.VALUES):
        """ Given a term, return a generator that generates PTs for that term not in the list."""
        self.slv.push()
        # declare input variables for the functions-to-synthesize
        principal = self.slv.mkConst(self.entity_sort, "principal")
        action = self.slv.mkConst( self.string_sort, "action")
        resource = self.slv.mkConst(self.entity_sort, "resource")
        ctx = self.slv.mkConst(self.context_sort, "ctx")
        # Principal resource range restrictions
        self.slv.assertFormula(self.slv.mkTerm(Kind.GEQ, self.id_of_entity(principal), self.slv.mkInteger(0)))
        self.slv.assertFormula(self.slv.mkTerm(Kind.LEQ, self.id_of_entity(principal), self.slv.mkInteger(self.entity_id_counter)))
        self.slv.assertFormula(self.slv.mkTerm(Kind.GEQ, self.type_of_entity(principal), self.slv.mkInteger(0)))
        self.slv.assertFormula(self.slv.mkTerm(Kind.LEQ, self.type_of_entity(principal), self.slv.mkInteger(self.type_id_counter)))
        ...
        self.slv.assertFormula(self.slv.mkTerm(Kind.GEQ, self.type_of_entity(resource), self.slv.mkInteger(0)))
        self.slv.assertFormula(self.slv.mkTerm(Kind.LEQ, self.type_of_entity(resource), self.slv.mkInteger(self.type_id_counter)))
        sat = term.substitute([self.rule_p, self.rule_a, self.rule_r, self.rule_ctx], [principal, action, resource, ctx])
        self.slv.assertFormula(sat)
        if self.verbose:
            print("Asserting: ", sat)
        # action need to be one of the actions
                ...
                for f, t in self.contexts:
                    ...
                    if v_is_null:
                        ...
                        if v.getSort() == self.entity_sort:
                            ...
                            context_dict[f] = v.toPythonObj()
                if mode is None:
                    log_entry((p_type, p_id), a, (r_type, r_id), context_dict, True)
                yield ((p_type, p_id), a, (r_type, r_id), context_dict), ()
            elif r2.isUnsat():
                # unsat_core = self.slv.getUnsatCore()
                self.slv.pop()
                return False, None
            else:
                raise Exception("Unknown result from SMT solver")

    def get_pt_generator(self, log, synth_rule, check_log=False, mode=cvc5.BlockModelsMode.VALUES):
        """ Given a rule, return a generator that generates PTs for that rule not in the log."""
        yield from self.get_pt_generator_term(log, self.encoded_rules[synth_rule], check_log, mode)


    def evaluate_req(self, synth_rule, p, a, r, ctx):
        """Evaluate the synthesized rule on a given request."""
        _ = self.slv.checkSat()
    ...
    def evaluate_term(self, term, p, a, r, ctx):
        """Evaluate a given term on a given request as a term."""
        _ = self.slv.checkSat()
        _ctx = self.parse_ctx(ctx)
        term_sub = term.substitute(
                        [self.rule_p, self.rule_a, self.rule_r, self.rule_ctx, self.principal, self.action, self.resource],
                        [self.entity(*p), self.slv.mkString(a), self.entity(*r), _ctx, self.entity(*p), self.slv.mkString(a), self.entity(*r)])
        truth = self.slv.getValue(term_sub)
        return truth.toPythonObj()
    ...
    def simplify_given_header(self, expr, synth_rule):
        """Best effort simplification of expression given the head of the rule.
        1. the type requirements of the rule head can be substituted as true and
        that of other types can be substituted as false.

        2. If the head allows one specific action, and the action only applies to
        one principal type or one resource type, then we can substitute those as
        true as well. Substitute false for restriction on other types.
        """
        
        principal = self.principal
        action = self.action
        resource = self.resource
        md = self.encoded_rules_metadata[synth_rule]

        # substitute header
        p_restrict = md["p_restrict"].substitute([self.rule_p], [principal])
        a_restrict = md["a_restrict"].substitute([self.rule_a], [action])
        r_restrict = md["r_restrict"].substitute([self.rule_r], [resource])
        cond = expr.substitute([self.rule_p, self.rule_a, self.rule_r], [principal, action, resource]).substitute([p_restrict], [self.slv.mkBoolean(True)]).substitute([a_restrict], [self.slv.mkBoolean(True)]).substitute([r_restrict], [self.slv.mkBoolean(True)])

        # substitue false for other type requirements
        for key, var in [("Principal", principal), ("Resource", resource)]:
            if md[key][0] == "Is":
                for type in self.entity_schema.keys():
                    if type == md[key][1]:
                        continue
                    term = self.slv.mkTerm(Kind.APPLY_UF, self.entity_is, var, self.slv.mkInteger(self.entity_type_map[type]))
                    cond = cond.substitute([term], [self.slv.mkBoolean(False)])

        # try to infer principal and resource types from the condition
        action_req = md['Action']
        if action_req[0] == 'Eq':
            action_applies_to = self.action_scope[action_req[1]]['appliesTo']
            principal_types = action_applies_to['principalTypes']
            resource_types = action_applies_to['resourceTypes']
            if len(principal_types) == 1:
                term = self.slv.mkTerm(Kind.APPLY_UF, self.entity_is, principal, self.slv.mkInteger(self.entity_type_map[principal_types[0]]))

                cond = cond.substitute([term], [self.slv.mkBoolean(True)])
                for type in self.entity_schema.keys():
                    if type == principal_types[0]:
                        continue
                    term = self.slv.mkTerm(Kind.APPLY_UF, self.entity_is, principal, self.slv.mkInteger(self.entity_type_map[type]))
                    cond = cond.substitute([term], [self.slv.mkBoolean(False)])
            if len(resource_types) == 1:
                term = self.slv.mkTerm(Kind.APPLY_UF, self.entity_is, resource, self.slv.mkInteger(self.entity_type_map[resource_types[0]]))
                cond = cond.substitute([term], [self.slv.mkBoolean(True)])
                for type in self.entity_schema.keys():
                    if type == resource_types[0]:
                        continue
                    term = self.slv.mkTerm(Kind.APPLY_UF, self.entity_is, resource, self.slv.mkInteger(self.entity_type_map[type]))
                    cond = cond.substitute([term], [self.slv.mkBoolean(False)])
            for act in self.actions:
                if act == action_req[1]:
                    continue
                term = self.slv.mkTerm(Kind.EQUAL, action, self.slv.mkString(act))
                cond = cond.substitute([term], [self.slv.mkBoolean(False)])

        simplified_cond = self.slv.simplify(cond)#.substitute([principal, action, resource], [self.rule_p, self.rule_a, self.rule_r])
